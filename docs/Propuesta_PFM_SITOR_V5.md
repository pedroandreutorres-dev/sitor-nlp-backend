...
* **Actualización de Viabilidad (Test A/B de Idioma):** Tras el Análisis Exploratorio de Datos (EDA), se ha detectado un volumen críticamente bajo de tickets nativos en español. Por tanto, el PFM ejecutará una fase de experimentación dual mediante dos *notebooks* paralelos:
  1. Un entrenamiento de la arquitectura con el dataset masivo nativo en **inglés** (~16.000 tickets útiles) para establecer un *baseline* de rendimiento NLP puro y viabilidad técnica de la arquitectura.
  2. Un entrenamiento evaluando una muestra **traducida automáticamente al español** utilizando modelos locales Open Source de HuggingFace (ej. Helsinki-NLP) para evitar costes y límites de API, con el objetivo de comprobar la degradación semántica introducida por la traducción de jerga técnica.
...
El sistema procesa el texto concatenado (Asunto + Cuerpo) del ticket para predecir la tripleta (aproximada como queue + type + priority en el dataset elegido).
...
Indexación de un subconjunto de tickets resueltos en una base vectorial (ChromaDB), recuperación de los casos más similares ante un ticket nuevo, y generación de una propuesta de resolución mediante un LLM (API de OpenAI u Ollama). *(Nota: Los datos para poblar el RAG se extraerían directamente de la capa Silver original, ya que las respuestas finales o 'answers' se purgarán de la capa Gold predictiva por eficiencia).*
...
* **Evaluación Demostrativa:** Dado que el dataset público solo contiene tipificaciones "correctas", la demostración en la defensa exigirá la **inyección de ruido sintético** (falsos escalados simulados) en el input para demostrar empíricamente la capacidad de intercepción del sistema.
...
* **Riesgo de la Arquitectura en Cascada (Contingencias):** Al contar con un corpus base acotado (~16.000 tickets útiles en inglés), existe riesgo de escasez de muestras en los subniveles profundos. Se mitigará con la técnica SMOTE. Si el rendimiento colapsa, se plantean tres escenarios: Plan A (Cascada completa de 3 niveles), Plan B (Acotar el MVP a los 2 primeros niveles) o Plan C (Predecir solo el nivel 1).
